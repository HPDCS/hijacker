	...
	while(sym) {
		...
		if (sym->type == SYMBOL_FILE) {
			...
			for (sec = hijacked.sections->next; sec; sec = sec->next) {
				if(sec->name != NULL && str_equal(sym->name, sec->name)) {
					sym->sec = sec;
